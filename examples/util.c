...
{
  int entries, row_count, row, column;
  char  buf[100];
  char  tbuf[100];
...
#define NUM_COLUMNS 8
  ...
  {
    snprintf(buf, sizeof(buf), "%04x ", row * NUM_COLUMNS);
    for (column = 0; (column < NUM_COLUMNS)
        && ((row * NUM_COLUMNS + column) < entries); column++)
    {
      snprintf(tbuf, sizeof(buf), "%8.8x ", array[row * NUM_COLUMNS + column]);
      strncat(buf, tbuf, strlen(tbuf));
    }
